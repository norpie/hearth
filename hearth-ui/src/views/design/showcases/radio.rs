use dioxus::prelude::*;
use crate::{Radio, RadioGroup, RadioSize, RadioDirection, Label};
...
pub fn radio_showcase(props: RadioShowcaseProps) -> Element {
    let RadioShowcaseProps { mut radio_theme, radio_size: _ } = props;
    rsx! {
        ComponentShowcase {
...
RadioGroup {
    ...
    Radio { value: "option1".to_string(), selected: selected.read().clone(), name: "options".to_string(), label: "Option 1".to_string() }
    Radio { value: "option2".to_string(), selected: selected.read().clone(), name: "options".to_string(), label: "Option 2".to_string() }
}"#.to_string(),
