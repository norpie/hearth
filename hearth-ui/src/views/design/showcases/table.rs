use dioxus::prelude::*;
use crate::components::{Table, TableHeader, TableBody, TableRow, TableHead, TableCell, TableVariant, TableSize, Badge, BadgeVariant};
...
pub fn TableShowcase() -> Element {
    // Sortable data state
    let mut employees = use_signal(|| vec![
        Employee {
    ...
    let mut sort_employees = move |field: SortBy| {
        let current_sort = sort_by.read().clone();
        let current_order = sort_order.read().clone();
    ...
    let mut get_sort_indicator = move |field: SortBy| {
        if sort_by.read().clone() == field {
            match sort_order.read().clone() {
                SortOrder::Asc => " ↑",
                SortOrder::Desc => " ↓",
            }
        } else {
